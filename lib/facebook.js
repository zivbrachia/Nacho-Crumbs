...
function setMessageTypeQuickReplies(title, replies) {
    ...
    for(let i=0; i<len; i++) {
        ...
        quick_reply.title = replies[i];
        quick_reply.payload = replies[i]; //"SOMETHING_SOMETHING";
...
function cardJson(messages, messageIndex) {
    let facebook = {
        attachment: {
            type: "template",
            payload: {
    ...
    while (message.type===1) {
        facebook.attachment.payload.elements.push(buildElement(messages[messageIndex]));
