...
function buildCurrentQuestion(session, promptObj) {
    ...
    for (let i = 0; i < session.userData.current_question.choices.length; i++) {
        ...
        arr_choice.push(session.userData.current_question.choices[i].labelHtml);
    }
...
function is_answer_correct(session, answer) {
    ...
    for (let i = 0; i < session.userData.current_question.choices.length; i++) {
        ...
        if ((session.userData.current_question.choices[i].title===answer) && (session.userData.current_question.choices[i].correct===true)) {
            return true;
        }
        //if ((session.userData.current_question.choices[i].title===answer) && (session.userData.current_question.choices[i].isCorrect===false)) {
        if ((session.userData.current_question.choices[i].title===answer) && (session.userData.current_question.choices[i].correct===false)) {
            return false;
        }
    }
    return false;
