...
module.exports = {sendMessages, getMessageImage, getMessageSourceEvent, getMessageText, getMessageTypeQuickReplies, proactive};
...
function buildQuestion(session, promptObj) {
    let messages = [];
    promptObj = promptObj || {};
    if ((session.userData.questions_backet === undefined) || (session.userData.questions_backet[0] === undefined)) {
...
function buildCurrentQuestion(session, promptObj) {
    ...
    if (session.userData.current_question.Open_Sentence && session.userData.current_question.Open_Sentence.length) {
        for (let i = 0; i < session.userData.current_question.Open_Sentence.length; i++) {
            if (session.userData.current_question.Open_Sentence[i].imageUrl) {
                let msg = new builder.Message(session).attachments([{"contentType": "image/gif", "contentUrl": session.userData.current_question.Open_Sentence[i].imageUrl}]);
                messages.push(msg);
            } else {
                if (session.userData.current_question.Open_Sentence[i].title!=="") {
    ...
    if (session.userData.current_question.Question && session.userData.current_question.Question.length) {
        for (let i = 0; i < session.userData.current_question.Question.length; i++) {
            if (session.userData.current_question.Question[i].imageUrl) {
                let msg = new builder.Message(session).attachments([{"contentType": "image/gif", "contentUrl": session.userData.current_question.Question[i].imageUrl}]);
                messages.push(msg);
            } else {
                if (session.userData.current_question.Question[i].title!=="") {
    ...
    if (is_change_choise_to_msg) {
        ...
        if (messages[messages.length - 1].data.text) {
            promptObj.title = messages[messages.length - 1].data.text;
            messages = messages.slice(0, messages.length-1);
        } else {
            promptObj.title = "מה התשובה?";
        }
    }
    //
    return messages;
...
function add_answers_to_msg(session) {
    ...
    for (let i = 0; i < session.userData.current_question.choices.length; i++) {
        ...
        session.userData.current_question.choices[i].title = (i + 1).toString();
    }
...
function is_answer_correct(session, answer) {
    //console.log("is_answer_correct", answer);
    if (answer===false) {
        return false;
    }
    for (let i = 0; i < session.userData.current_question.choices.length; i++) {
...
bot.dialog('question_flow', [
    ...
    function (session, results, next) { // answer right - next question # answer wrong - send hint dialog
        ...
        let promptObj = {};
        console.log("answer right - next question # answer wrong - send hint dialog 1", promptObj);
        ...
        console.log("answer right - next question # answer wrong - send hint dialog 2", answer_status);
        if (answer_status === true) {
            ...
            if (promptObj.end_chapter) {
                ...
                console.log("answer right - next question # answer wrong - send hint dialog", promptObj);
                builder.Prompts.choice(session, promptObj.title, promptObj.choice, { listStyle: builder.ListStyle.button, maxRetries: 0 });
            ...
            if ((session.userData.hint_try >= 1) || (!session.userData.current_question.Hint)) {
                let messages = build_answer_messages(session, answer_status, promptObj);
                 send_messages(session, messages);
                 if (promptObj.end_chapter) {
                ...
                if (session.userData.current_question.Hint && session.userData.current_question.Hint.length) {
...
bot.dialog("error_flow", [
    function (session, args, next) {
        ...
        builder.Prompts.choice(session, "אוי זה ממש מביך, אולי תנסה שוב", "בכיתה|בבית", {listStyle: builder.ListStyle.button, maxRetries:0});
...
bot.dialog('practice_specific_chapter', [
    function (session) {    // practice specific chapter per the study chapter
        session.sendTyping();
        session.userData.learn = "practice";
        let text_arr = session.message.text.split(" ");//chapters
        if (text_arr.length===2) {
            session.userData.chapter = text_arr[1];
        } else if (text_arr.length===3) {
            session.userData.chapter = text_arr[2];
        }
        
        //cet_by_document("4d8dd634-2569-45d8-9cce-a4a83ae625ce", function (err, questions_backet) {
        cet_by_document(chapters[session.userData.chapter-1].cet.document.practice, function (err, questions_backet) {
            if (err) {
    ...
    matches: [/^תרגול [0-9]+$/i, /^תרגול פרק [0-9]+$/i, /^בבית [0-9]+$/i, /^בבית פרק [0-9]+$/i]
});
...
bot.dialog('chapter', [
    function (session) {    // loads the question per chapter
        ...
        if (chapter === undefined) {
            ...
            cet_by_document(chapters[session.userData.chapter - 1].cet.document[session.userData.learn], function (err, questions_backet) {
                if (err) {
                    session.replaceDialog("error_flow");
                }
                else {
...
function parseDocumentRevisions(documentRevisions) {
    ...
    documentRevisions.documentModel.e_questionnaire.e_page.forEach(function(page) {
        let question = {};
        if (page.e_question[0]===undefined) {
            return;
        }
        ...
        if (page.e_question[0].instructions) {
            ...
            question.Question = messages_from_cet(arr_tags);
        ...
        if (page.e_question[0].e_questionAssistants.assistant3) {
            ...
            question.Hint = messages_from_cet(arr_tags);
        ...
        if (page.e_question[0].e_questionAssistants.assistant2) {
            ...
            question.Explain_Sentence = messages_from_cet(arr_tags);
            /*for (let i=0; i < arr_tags.length; i++) {
                if (arr_tags[i]) {
        ...
        if (page.e_staticContent && page.e_staticContent[0] && page.e_staticContent[0].htmlContent) {
            ...
            question.Open_Sentence = messages_from_cet(arr_tags);
            /*for (let i=0; i < arr_tags.length; i++) {
                if (arr_tags[i]) {
...
function messages_from_cet(arr_tags) {
    let messages = [];
    let title = "";
    for (let i=0; i < arr_tags.length; i++) {
        if (arr_tags[i].includes("<img")) {
            if (title && title!=="\n") {
                let msg = {"title": title};
                messages.push(msg);
                title = "";
            }
            let msg = { "imageUrl": arr_tags[i].split('"')[1] };
            messages.push(msg);
        }
        else {
            title = (title || "") + arr_tags[i];
        }
    }
    if (title && title!=="\n") {
        let msg = { "title": title };
...
function send_chapters(session) {
    ...
    chapters.forEach(function (element, i) {
        chapter.push(getCard(session, element.title, '', '', element.pic, [{ "postback": "פרק " + (element.id), "title": "פרק " + (element.id) }]));
    });
...
function endChapter(session, promptObj) {
    ...
    let text_title = "";
    let text_next = "";
    session.userData.chapter = session.userData.chapter || "";   // default
    session.userData.chapter_name = session.userData.chapter_name || "";   // default
    if (session.userData.learn==="study") {
        //text_title = "סיימנו את חלק הלימוד של פרק";
        text_title = "סיימנו את חלק הכיתתי של פרק";
        //text_next = "עכשיו אפשר לתרגל את הפרק או ללמוד פרק חדש?"
        text_next = "עכשיו אפשר לתרגל בבית את הפרק או ללמוד בכיתה פרק נוסף?"
        //promptObj.choice = "תרגול פרק" + " " + session.userData.chapter + "|" + "לימוד";
        promptObj.choice = "תרגול פרק" + " " + session.userData.chapter + "|" + "בכיתה";
    } else {
        text_title = "סיימנו את חלק התרגול של פרק"
        //promptObj.choice = "תרגול|לימוד";
        promptObj.choice = "בבית|בכיתה";
    }
    ...
    if (session.userData.current_question.Explain_Sentence_Before && session.userData.current_question.Explain_Sentence_Before.length) {
        for (let i=0; i<session.userData.current_question.Explain_Sentence_Before.length; i++) {
            if (session.userData.current_question.Explain_Sentence_Before[i].title!=="") {
                console.log("Explain_Sentence_Before", session.userData.current_question.Explain_Sentence_Before[i].title);
    ...
    if (session.userData.current_question.Explain_Sentence_After && session.userData.current_question.Explain_Sentence_After.length) {
        for (let i=0; i<session.userData.current_question.Explain_Sentence_After.length; i++) {
            if (session.userData.current_question.Explain_Sentence_After[i].title!=="") {
                console.log("Explain_Sentence_After", session.userData.current_question.Explain_Sentence_After[i].title);
...
function hint(session, promptObj) {
    ...
    if (session.userData.current_question.Hint && session.userData.current_question.Hint.length) {
        for (let i=0; i<session.userData.current_question.Hint.length; i++) {
            if (session.userData.current_question.Hint[i].imageUrl) {
                let msg = new builder.Message(session).attachments([{"contentType": "image/gif", "contentUrl": session.userData.current_question.Hint[i].imageUrl}]);
                messages.push(msg);
            }
            else {
                if (session.userData.current_question.Hint[i].title!=="") {
...
function getMessageTypeQuickReplies(address, title, replies) {
    let actions_1 = [];
    ...
    let keyboard = new builder.Keyboard().buttons(actions_1);
    let msg = new builder.Message().address(address).text(title).addAttachment(keyboard);
    return msg;
}
...
function proactive(source, userId, text) {
    db.getAddressByUserId(source, userId, function (address) {
