...
function buildCurrentQuestion(session, promptObj) {
    ...
    console.log("is_change_choise_to_msg", is_change_choise_to_msg);
    if (is_change_choise_to_msg) {
        let obj_choice = { "arr_choice": arr_choice }
        console.log("is_change_choise_to_msg 1", obj_choice);
        let msg = add_answers_to_msg(session, obj_choice);
        console.log("is_change_choise_to_msg 2", obj_choice);
        messages.push(msg);
    }
...
function add_answers_to_msg(session, obj_choice) {
    let msg_text = "";
    let choices_new = [];
    for (let i = 0; i < obj_choice.arr_choice.length; i++) {
        msg_text = (msg_text || "") + (i + 1) + ". " + obj_choice.arr_choice[i].title + "\n";
        let choise = { "title": "", "isCorrect": "" };
        choise.title = (i + 1);
        choise.isCorrect = obj_choice.arr_choice[i].isCorrect;
        choices_new.push(choise);
    }
    let msg = new builder.Message(session).text(msg_text);
    obj_choice.arr_choice = choices_new;
    console.log("add_answers_to_msg", obj_choice.arr_choice);
    return msg;
}
