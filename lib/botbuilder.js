...
function endChapter(session) {
    ...
    if (session.userData.learn==="study") {
        ...
        text_next = "עכשיו נתרגל?"
        sourceEvent.facebook.quick_replies.push({ "content_type": "text", "title": "כן", "payload": "תרגול " + session.userData.chapter });
        sourceEvent.facebook.quick_replies.push({ "content_type": "text", "title": "לא", "payload": "לא" });
    } else {
    ...
    let msg = new builder.Message(session).text(text_title + " " + session.userData.chapter);
    messages.push(msg);
    if (text_next) {
...
intents.matches(/^המשך/i, function (session) {
    session.sendTyping();
...
function afterAnswer(session) {
    ...
    if (session.userData.questions_backet[0]===undefined) { // after delete the current question, we look at the future question.
        let end_messages = endChapter(session);
        for (let i=0;i<end_messages.length;i++) {
            messages.push(end_messages[i])    
        }
    }
    else {
        sourceEvent.facebook.quick_replies.push({ "content_type": "text", "title": "המשך", "payload": "המשך" });
        messages[messages.length - 1].sourceEvent(sourceEvent);
    }
    return messages;
}
...
intents.matches(/^לימוד/i, function (session) {
    ...
    db.getChapters(function (chapters, res) {
        ...
        chapters.forEach(function (element, i) {
            chapter.push(getCard(session, element.title, '', '', '', [{"postback": "פרק " + (4), "title": "פרק " + (element.id)}]));
        });
...
intents.matches(/^תרגול [0-9]+$/i, function (session) {
    session.sendTyping();
    session.userData.learn = "practice";
    session.userData.chapter = 4;//session.message.text.split(" ")[1];
    ...
    db.getQuestionsByChapter(session.userData.chapter, function (questions) {
        ...
        let replyMessage = new builder.Message(session).text("?נתחיל לתרגל");
        replyMessage.sourceEvent({
            facebook: {
                quick_replies: [{
                    content_type: "text",
