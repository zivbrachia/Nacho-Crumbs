...
function buildCurrentQuestion(session, promptObj) {
    ...
    promptObj.title = session.userData.current_question.Question;
...
function is_answer_correct(session, answer) {
    ...
    for (let i = 0; i < session.userData.current_question.choices.length; i++) {
        if ((session.userData.current_question.choices[i].title===answer) && (session.userData.current_question.choices[i].isCorrect===true)) {
            return true;
        }
        if ((session.userData.current_question.choices[i].title===answer) && (session.userData.current_question.choices[i].isCorrect===false)) {
            return false;
        }
    }
    return false;
    ...
    function (session) {    // loads the question per chapter
        ...
        if (chapter === undefined) {
            ...
            db.getQuestionsByChapter(session.userData.chapter, function (questions) {
                ...
                //builder.Prompts.choice(session, "?מתחילים", "התחל", {listStyle: builder.ListStyle.button, maxRetries:0});
...
function cet_by_document(item_id) {
    ...
    client.get('/api/DocumentVersions/' + item_id + '/he', function (err, request, response, obj) {
        ...
        client.get('/api/DocumentRevisions/' + item_id + '/he/' + majorVersion + '/' + minorVersion, function (err, request, response, obj) {
            ...
            //
            let snap = buildSnapFromCet(questions_backet);
...
function parseDocumentRevisions(documentRevisions) {
    ...
    documentRevisions.documentModel.e_questionnaire.e_page.forEach(function(page) {
        ...
        question.Question = page.e_question[0].instructions;
        question.Hint[0] = page.e_question[0].e_question[0].e_questionAssistants.assistant2;
        question.Explain_Sentence[0] = page.e_question[0].e_question[0].e_questionAssistants.assistant3;
        question.Open_Sentence[0] = page.e_staticContent[0].htmlContent;
        question.choices = page.e_question[0].e_singleChoice.e_option;
...
function endChapter(session, promptObj) {
    ...
    if (session.userData.learn==="study") {
        ...
        promptObj.choice = "לימוד"+"|"+"תרגול פרק" + " " + session.userData.chapter;
    } else {
        text_title = "סיימנו את חלק התרגול של פרק"
        promptObj.choice = "לימוד|תרגול";
    }
    let msg = new builder.Message(session).attachments([{ "contentType": "image/gif", "contentUrl": "https://firebasestorage.googleapis.com/v0/b/nacho-crumbs.appspot.com/o/chapters%2Fend_perek_pic.gif?alt=media&token=664edf0f-315e-410c-91ac-408a9cfda2ad" }]);
    messages.push(msg);
...
function hint(session, promptObj) {
    ...
    let arr_choice = [];
    for (let i = 0; i < session.userData.current_question.choices.length; i++) {
        if (session.userData.current_question.choices[i].title !== wrong_answer)
            arr_choice.push(session.userData.current_question.choices[i].title);
    }
