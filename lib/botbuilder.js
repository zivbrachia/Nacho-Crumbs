...
intents.matches(/^פרק [א-ת]+$/i, function (session) {
    session.sendTyping();
    //
...
intents.matches(/^trueAnswer/i, function (session) {
    ...
    let msg = new builder.Message(session).text(reply.right_answer.Math.floor(Math.random()*reply.right_answer.length));
