...
function buildCurrentQuestion(session, promptObj) {
    ...
    for (let i = 0; i < session.userData.current_question.choices.length; i++) {
        ...
        if (session.userData.current_question.choices[i].title >= 20) {
            is_change_choise_to_msg = true;
        }
    }
    console.log("is_change_choise_to_msg", is_change_choise_to_msg);
    if (is_change_choise_to_msg) {
        console.log("is_change_choise_to_msg 1", arr_choice);
        let msg = add_answers_to_msg(arr_choice);
        console.log("is_change_choise_to_msg 2", arr_choice);
        messages.push(msg);
    }
...
function add_answers_to_msg(arr_choice) {
    let msg_text = "";
    let choices_new = [];
    for (let i = 0; i < arr_choice.length; i++) {
        msg_text = (msg_text || "") + (i + 1) + ". " + arr_choice.title + "\n";
        let choise = { "title": "", "isCorrect": "" };
        choise.title = (i + 1);
        choise.isCorrect = question.choices[i].isCorrect;
        choices_new.push(choise);
    }
    let msg = new builder.Message(session).text(msg_text);
    arr_choice = choices_new;
    console.log("add_answers_to_msg", [arr_choice]);
    return msg;
}
