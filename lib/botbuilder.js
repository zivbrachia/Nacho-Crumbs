...
bot.dialog('study', [
    function (session) {
        ...
        db.getChapters(function (chapters, session) {
            let chapter = [];
            chapters.forEach(function (element, i) {
                chapter.push(getCard(session, element.title, '', '', element.pic, [{ "postback": "chapter " + (4), "title": "chapter " + (element.id) }]));
            });
...
function endChapter(session, promptObj) {
    ...
    if (session.userData.learn==="study") {
        ...
        promptObj.choice = "לימוד"+"|"+"תרגול" + " " + session.userData.chapter;
    } else {
        text_title = "סיימנו את חלק התרגול של פרק"
        promptObj.choice = "לימוד|תרגול";
    }
    let msg = new builder.Message(session).attachments([{ "contentType": "image/gif", "contentUrl": "https://firebasestorage.googleapis.com/v0/b/nacho-crumbs.appspot.com/o/chapters%2Fend_perek_pic%402x-100.jpg.jpg?alt=media&token=df09380f-a108-41b5-9f7b-80ea3bcd4686" }]);
    messages.push(msg);
    msg = new builder.Message(session).text(text_title + " " + session.userData.chapter);
    messages.push(msg);
