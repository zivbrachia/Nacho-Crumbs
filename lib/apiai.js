...
module.exports = {app, getAllIntents, createQuestionSession, createInfoIntent, 
                    ...
                    getMessageTypeText, getMessageTypeQuickReplies};

apiaiEventEmitter.on('apiai_response', function (connObj, response, userData, source) {
        setImmediate(function() {
            controller.chatFlow1(connObj, response, userData, source);
        });
});
...
function createIntentQuestionAsk(questionNumber, req_params) { //Category, SubCategory, questionText, rightAnswer, wrongAnswer1, wrongAnswer2, wrongAnswer3) {
    ...
    let question = require('../apiai_template/intents/New_Question_Ask_X.js').questionTemplate(questionNumber, req_params.category, req_params.sub_category, req_params.question, newArr[0], newArr[1], newArr[2], newArr[3]);
    return question;
}

function createIntentQuestionAskPromise(questionNumber, req_params) {
    let questionAsk = new Promise(function (resolve, reject) {
        ...
        let questionIntent = createIntentQuestionAsk(questionNumber, req_params);
...
function createIntentFollowUpPromise(questionNumber, req_params, parentId, rootParentId) { 
    let followUp = new Promise(function (resolve, reject) {
        ...
        let intentArray = createFollowUpIntents(questionNumber, req_params, parentId, rootParentId);
...
function createFollowUpIntents(questionNumber, req_params, parentId, rootParentId) {
    let intentArray = [];
    ...
    let hintIntent = createIntentHint(questionNumber, req_params.hint, parentId, rootParentId);
    let fallbackIntent = createIntentFallback(questionNumber, parentId, rootParentId, req_params.hint);
...
function createIntentFallback(QuestionNumber, parentId, rootParentId, speech) {
    let fallback = require("../apiai_template/intents/New_Question_Ask_X-fallback.js").fallbackTemplate(QuestionNumber, parentId, rootParentId, speech);
    //
    return fallback;
}
...
function createIntentHint(QuestionNumber, hintText, parentId, rootParentId) {
    //let hint = require("../apiai_template/intents/Question_Hint_X.js").hintTemplate(QuestionNumber, hintText);
    let hint = require("../apiai_template/intents/New_Question_Ask_X-hint.js").hintTemplate(QuestionNumber, parentId, rootParentId, hintText);
    //
    return hint;
}
...
function getMessage(address, message, builder, textData, messageIndex) {
    let msg = {};
    switch (message.type) {
        case 0:
            ...
            break;
        case 1:
            msg.msg = getMessageTypeCard(address, builder, textData.messages, messageIndex);
            break;
        case 2:
...
function getMessageTypeCard(address, builder, messages, messageIndex) {
    let msg = {};
    if (address.channelId==='facebook') {
        msg = getMessageTypeCustomPayload(address, facebook.cardJson(messages, messageIndex), builder);
    }
    else if (address.channelId==='telegram') {
        msg = getMessageTypeCustomPayload(address, telegram.cardJson('1111', 'מידע'), builder);
    }
    return msg;
}
...
function buildMessages(response, address, source) {
    ...
    for (let i=0; i<(len); i++) {
        ...
        msg = getMessage(address, apiaiMessage, builder, textData, i);
