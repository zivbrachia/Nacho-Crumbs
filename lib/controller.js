...
function chatFlow1(connObj, response, userData, source, promises) {
    ...
    if (response.result.action==="input.study") {
        userData.learn = "study";
        //let msg2 = require('../lib/apiai.js').getMessageTypeQuickReplies(address, "?יאללה מתחילים", ["התחל"], builder);
        //messages.push(msg2.data);
        return require('../lib/apiai.js').eventRequest('Chapters', address, 0, userData || {});
    }   
    if (response.result.action==="input.practice") {
        userData.learn = "practice";
        let msg2 = require('../lib/apiai.js').getMessageTypeQuickReplies(address, "יאללה מתחילים?", ["התחל"], builder);
        messages.push(msg2.data);
    }
    ...
    if (isNext(response.result.action)) {
        userData.question = userData.question || {};
        let nextQuestion = getNextQuestion(userData.question.intentName, userData.learn);
...
function getNextQuestion(current_question, learn) {
    let nextQuestion;
    let questions_practice = {
        Begin : 'Question_Ask_150',
        Question_Ask_150 : 'Question_Ask_151',
        Question_Ask_151 : 'Question_Ask_152',
        Question_Ask_152 : 'Question_Ask_153',
        Question_Ask_153 : 'Question_Ask_150',
    }
    let questions = {
        Begin : 'Question_Ask_134',
        /* Day 1 */
        ...
        /* Day 4 */
        Question_Ask_149 : 'Question_Ask_134'
        //Question_Ask_150 : 'Question_Ask_151',
        //Question_Ask_151 : 'Question_Ask_152',
        //Question_Ask_152 : 'Question_Ask_153',
    ...
    if (current_question===undefined) {
        ...
        else if (learn==="practice") {
            nextQuestion = questions_practice['Begin'];
        } else {
            nextQuestion = questions['Begin'];
        }
    } else {
